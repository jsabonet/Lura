    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="relative">
        ...
        <button
          ...
          className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-left flex items-center justify-between hover:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-colors"
        >
          <span className={currentProvince ? 'text-gray-900' : 'text-gray-500'}>
      ...
      {/* Seletor de Distrito - só aparece se uma província estiver selecionada */}
      {currentProvince && (
        <div className="relative">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            <TreePine className="inline w-4 h-4 mr-1" />
            Distrito (Opcional - para dados mais específicos)
          </label>
          <button
          ...
          >
            <span className={selectedDistrict ? 'text-gray-900' : 'text-gray-500'}>
              {selectedDistrict || 'Selecione um distrito (opcional)'}
            </span>
            <ChevronDown 
              className={`w-5 h-5 text-gray-400 transition-transform ${
