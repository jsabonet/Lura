  ...
  cropIds.forEach(cropId => {
    ...
    if (compatibility === 'incompatible') {
      ...
      insights.push({
        ...
        description: `${crop.name} pode ser cultivado em ${province}, mas requer maneio especial. Monitore temperatura e irrigação cuidadosamente.`,
        priority: 'medium',
        crops: [crop.name],
        actionable: true,
        severity: 'medium'
...
function getGeneralWeatherInsights(
  ...
  if (season === 'wet') {
    ...
    insights.push({
      ...
      category: 'irrigation',
      icon: '☀️',
      title: 'Estação Seca - Maneio Hídrico',
      description: `Foque em conservação de água e culturas resistentes à seca. Optimize sistemas de rega.`,
      priority: 'high',
      actionable: true,
      timeline: 'Esta estação'
    });
